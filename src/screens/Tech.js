...
const Tech = ({ navigation }) => {
    ...
    useEffect(() => {
        async function fetchData() {
            const response = await fetch(`https://newsapi.org/v2/top-headlines?category=technology&country=in&apiKey=${API_KEY}`);
    ...
    const onRefresh = async () => {
        setRefreshing(true);
        const response = await fetch(`https://newsapi.org/v2/top-headlines?category=sports&country=in&apiKey=${API_KEY}`);
        if (response.status === 200) {
            const json = await response.json();
            // console.log("Total results: "+json.totalResults);
            setNews(json.articles);
